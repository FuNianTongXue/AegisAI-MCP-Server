...
随着 MCP 服务被用于聊天通讯、代码分析、威胁情报、移动应用测试、逆向工程和安全扫描，智能体已经能够从“回答问题”进一步执行查询、读取文件、调用 API、运行命令、控制设备和生成报告。能力越强，服务接入、第三方依赖、数据流向和工具授权带来的风险越需要被系统管理。

本项目成立的原因，是为安全智能体驱动的服务建设提供一个**可检索、可复核、可治理的参考目录**：帮助使用者先理解服务能做什么、需要什么权限、可能处理哪些数据，再决定是否安装、下载或接入；同时推动服务清单、版本固定、SBOM、门闸审批、沙箱执行和持续监控成为默认实践。本项目不鼓励盲目安装，也不对目录中的第三方项目作安全背书。

## 安全智能体驱动服务的参考方向

安全智能体不应被设计成拥有无限权限的自动化账号，而应被设计成受到边界约束、可审计、可回滚的编排层。推荐将“模型决策、MCP 工具、执行沙箱、数据访问代理和审批审计”分层部署，并让高风险动作经过人工确认或策略引擎批准。

| 参考方向 | 建议做法 |
|---|---|
| 服务发现与资产管理 | 为每个 MCP 服务建立资产编号，登记上游、版本、提交哈希、依赖、镜像、工具清单、端点、数据区域和责任人 |
| 安全接入门闸 | 对服务来源、许可证、哈希、漏洞、权限、数据流、网络端点和工具能力进行分层校验；未知服务默认拒绝 |
| 最小权限执行 | 使用工具级白名单、文件目录白名单、短期凭据、只读 API、出站域名白名单和人工确认，禁止默认自动批准高风险工具 |
| 高风险隔离 | 对逆向、APK/IPA 分析、恶意样本、命令执行、不可信代码和依赖构建使用 E2B 等临时安全沙箱或组织批准的等效环境 |
| 数据与跨境治理 | 对代码、消息、漏洞报告、应用包、样本、IOC 和个人信息进行分级，核验境外处理、第三方共享、留存和删除机制 |
| 可观测与响应 | 记录提示、工具调用、输入输出摘要、网络连接、凭据使用、异常行为和审批记录；发生事件时可停用、撤销和重建服务 |
| 结果复核与回滚 | 将智能体输出视为建议和证据线索，由安全、业务或合规责任人复核；对消息发送、PR 发布、配置修改和数据删除提供回滚能力 |

更具体的 SBOM 字段、风险分级、拒绝条件和沙箱控制要求见 [`docs/security-governance.md`](docs/security-governance.md) 与 [`data/mcp-sbom-template.csv`](data/mcp-sbom-template.csv)。

## 加入群聊交流

欢迎对 MCP 安全接入、智能体驱动安全服务、移动安全、威胁情报、逆向工程、合规检测和供应链治理感兴趣的开发者、安全工程师、研究人员与使用者加入群聊，共同讨论服务评估、沙箱实践、SBOM 管理、门闸策略、误报处理和生产落地经验。

<p align="center">
  <img src="assets/aegisai-group-qr.jpg" alt="群聊：神盾 AegisAI 智能助手" width="420">
</p>

<p align="center"><strong>群聊：神盾 AegisAI 智能助手</strong><br>二维码有效期：7 天内（9 月 11 日前）有效，重新进入将更新。</p>

请在扫码前确认二维码来源和有效期；群聊交流不代表对任何 MCP 服务、第三方平台或安全结论的认可。请勿在群内发送 API 密钥、个人信息、生产凭据、未公开漏洞、真实客户数据或恶意样本。
